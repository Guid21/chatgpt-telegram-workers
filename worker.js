...
var ENV = {
  ...
  async function defaultIndexAction() {
    const HTML = renderHTML(`
      ...
      <p>После привязки вебхука вы можете использовать следующие команды для управления ботом:</p>
      ${commandsDocument().map((item) => `<p><strong>${item.command}</strong> - ${item.description}</p>`).join("")}
      <br/>
      <p>Вы можете получить информацию о боте, посетив следующий URL-адрес:</p>
      <p><strong>/telegram/:token/bot</strong> - Получить информацию о боте</p>
      ${footer}
    `);
    return new Response(HTML, { status: 200, headers: { "Content-Type": "text/html" } });
  }
  ...
  var en_default = {
    ...
    command: {
      help: {
        ...
        "new": "Начните новый разговор",
        "start": "Получите ID и начните новый разговор",
        "img": "Создать изображение, формат команды следующий `/img описание изображения`, например `/img пляж при свете луны`",
        "version": "Получить номер текущей версии для определения необходимости обновления",
        "setenv": "Установите пользовательскую конфигурацию,  формат команды /setenv KEY=VALUE",
        "delenv": "Удаление пользовательской конфигурации, формат команды /delenv KEY",
        "usage": "Получение текущей статистики использования робота",
        "system": "Просмотр некоторой информации о системе",
        "role": "Установите идентификатор предустановки",
        "redo": "Повторить последний разговор, /redo с измененным содержимым или непосредственно /redo",
  ...
  `,
        "help": "Ошибка формата: полный формат команды `/role operation`\nВ настоящее время поддерживаются следующие `операции`:\n `/role show` Отображение текущих определенных ролей.\n `/role role name del` Удаление указанной роли.\n `/role role name KEY=VALUE` Установка конфигурации указанной роли.\nВ настоящее время поддерживаются следующие настройки:\n `SYSTEM_INIT_MESSAGE`: Сообщение об инициализации\n `OPENAI_API_EXTRA_PARAMS`: Дополнительные параметры API OpenAI, должны быть в формате JSON",
        "delete_role_success": "Роль удалена успешно",
        "delete_role_error": (e) => `Ошибка удаления роли: \`${e.message}\``,
        "update_role_success": "Конфигурации успешно обновлены",
        "update_role_error": (e) => `Сообщение ошибки: \`${e.message}\``
      },
      img: {
        "help": "Введите описание изображения. Формат команды следующий`/img енот-кошка`"
      },
      new: {
        "new_chat_start": "Начался новый разговор",
        "new_chat_start_private": (id) => `Начался новый разговор, ваш ID (${id})`,
        "new_chat_start_group": (id) => `Начался новый разговор, группа ID (${id})`
      },
      setenv: {
  ...
  - Per chat usage: `,
      ...
      permission: {
        "not_authorized": "Проверка разрешения на идентификацию не прошла",
        "not_enough_permission": (roleList, chatRole) => `Недостаточно прав, необходимо ${roleList.join(",")}, текущая роль: ${chatRole}`,
        "role_error": (e) => `Ошибка проверки личности: ` + e.message,
